...
<div class="max-w-3xl" x-data="{ loading: false }">
    ...
    <form method="POST" action="{{ route('admin.karyawan.store') }}" @submit="loading = true">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="alamat" name="alamat" rows="2"
                              class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ old('alamat') }}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select id="pt_klien_id" name="pt_klien_id" required
                            class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none @error('pt_klien_id') border-red-300 @enderror">
                        <option value="">Pilih PT Klien</option>
                        @foreach($ptKliens ?? [] as $pt)
                            <option value="{{ $pt->id }}" {{ old('pt_klien_id') == $pt->id ? 'selected' : '' }}>{{ $pt->nama }}</option>
                        @endforeach
                    </select>
                    @error('pt_klien_id')<p class="text-sm text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <input type="text" id="jabatan" name="jabatan" value="{{ old('jabatan') }}" required
                           class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none @error('jabatan') border-red-300 @enderror">
                    @error('jabatan')<p class="text-sm text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <input type="number" id="gaji_pokok" name="gaji_pokok" value="{{ old('gaji_pokok') }}" required min="0"
                           class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none @error('gaji_pokok') border-red-300 @enderror">
                    @error('gaji_pokok')<p class="text-sm text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="tanggal_bergabung" class="block text-sm font-medium text-slate-700 mb-1">Tanggal Bergabung <span class="text-red-500">*</span></label>
                    <input type="date" id="tanggal_bergabung" name="tanggal_bergabung" value="{{ old('tanggal_bergabung') }}" required
                           class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                </div>
                <div>
